...
int main()
{
	...
	// Create a *.csv
	ofstream myFile;
	myFile.open("output_GroundTruth_data.csv");
	string headers = "Heat,Cool,Weather_Outside,Raw_Temp.";
	myFile << headers << endl;

	while(1)
	{
		pthread_mutex_lock(&bufferLock);
		myFile << stations_data[0].digitalOut[0] << "," << stations_data[0].digitalOut[1] << "," << stations_data[0].digitalOut[2] << "," << stations_data[0].analogIn[0] << endl;
		pthread_mutex_unlock(&bufferLock);
