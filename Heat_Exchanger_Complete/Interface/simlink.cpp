...
int main()
{
	...
	// Create a *.csv
	ofstream myFile;
	myFile.open("output_GroundTruth_HEATEXCHANGER_data.csv");
	string headers = "ColdTempIn(inputRegister),ColdTempOut(inputRegister),HotMassFlowIn(inputRegister),HotTempIn(inputRegister),ShellPress(inputRegister),Temperature(inputRegister),TubePress(inputRegister)";
	myFile << headers << endl;
	
	while(1)
	{
		pthread_mutex_lock(&bufferLock);
		myFile << stations_data[0].analogIn[1] << "," << stations_data[0].analogIn[4] << "," << stations_data[0].analogIn[3] << "," << stations_data[0].analogIn[2] << "," << stations_data[0].analogIn[6] << "," << stations_data[0].analogIn[0] << "," << stations_data[0].analogIn[5] << endl;
		pthread_mutex_unlock(&bufferLock);
