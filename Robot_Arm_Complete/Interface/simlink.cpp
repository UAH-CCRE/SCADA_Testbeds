...
int main()
{
	...
	// Create a *.csv
	ofstream myFile;
	myFile.open("output_GroundTruth_ROBOTARM_data.csv");
	string headers = "Tau1(inputRegister),Tau2(inputRegister),Theta1(inputRegister),Theta2(inputRegister)";
	myFile << headers << endl;

	while(1)
	{
		pthread_mutex_lock(&bufferLock);
		myFile << stations_data[0].analogIn[0] << "," << stations_data[0].analogIn[1] << "," << stations_data[0].analogIn[2] << "," << stations_data[0].analogIn[3] << endl;
		pthread_mutex_unlock(&bufferLock);
