...
void *exchangeDataWithPLC(void *args)
{
	...
	while (1)
	{
		...
		//printf("Sending %d bytes to station: %d\n", sizeof(struct plcData), stationNumber);
...
int main()
{
	...
	// Create a *.csv
	ofstream myFile;
	myFile.open("output_GroundTruth_LOCK_data.csv");
	string headers = "Upper_Gate_Open_Command(coil),Upper_Gate_Close_Command(coil),Lower_Gate_Open_Command(coil),Lower_Gate_Close_Command(coil),";
	headers += "Upper_Valve_Open_Command(coil),Upper_Valve_Close_Command(coil),Lower_Valve_Open_Command(coil),Lower_Valve_Close_Command(coil),";
	headers += "Warn_Button(coil),Emergency_Gate(coil),Emergency_Valve(coil),Reservoir_and_Chamber_Depths_Equal(coil),";
	headers += "Tailwater_and_Chamber_Depths_Equal(coil),Flow_In(inputRegister),Flow_Out(inputRegister),Upper_Gate_Value(inputRegister),";
	headers += "Lower_Gate_Value(inputRegister),Upper_Valve_Value(inputRegister),Lower_Valve_Value(inputRegister),Chamber_Depth(inputRegister),";
	headers += "Reservoir_Depth(inputRegister),Tailwater_Depth(inputRegister),Sill_Height(inputRegister)";
	myFile << headers << endl;
	
	while(1)
	{
		pthread_mutex_lock(&bufferLock);
		myFile << stations_data[0].digitalOut[0] << "," << stations_data[0].digitalOut[1] << "," << stations_data[0].digitalOut[2];
		myFile << stations_data[0].digitalOut[3] << "," << stations_data[0].digitalOut[4] << "," << stations_data[0].digitalOut[5];
		myFile << stations_data[0].digitalOut[6] << "," << stations_data[0].digitalOut[7] << "," << stations_data[0].digitalOut[8];
		myFile << stations_data[0].digitalOut[9] << "," << stations_data[0].digitalOut[10] << "," << stations_data[0].digitalOut[12];
		myFile << stations_data[0].digitalOut[12] << "," << stations_data[0].analogIn[0] << "," << stations_data[0].analogIn[1];
		myFile << stations_data[0].analogIn[2] << "," << stations_data[0].analogIn[3] << "," << stations_data[0].analogIn[4];
		myFile << stations_data[0].analogIn[5] << "," << stations_data[0].analogIn[6] << "," << stations_data[0].analogIn[7];
		myFile << stations_data[0].analogIn[8] << "," << stations_data[0].analogIn[9] << endl;
		pthread_mutex_unlock(&bufferLock);
